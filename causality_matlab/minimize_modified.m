function [X, fX, i,nlml_vector,hyp_vector] = minimize_modified(X, f, length, varargin)
...
% disp(['nlml value: ',num2str(f0),' and gradient: ',num2str(df0')])
nlml_vector(:,end+1) = [f0;X;df0]; % function value, parameters, derivatives
hyp_vector(:,end+1) = X;
...
x3 = 10*red/(1-d0);                                  % initial step is red/(|s|+1)
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        ...
        df3 = unwrap(df3);
        % disp(['nlml value: ',num2str(f3),' and gradient: ',num2str(df3')])
        nlml_vector(:,end+1) = [f3;X+x3*s;df3]; % f val, params, derivatives
...
%   if f3 < f0+x3*RHO*d0          % MODIFIED!!!!!!!!!!!! % if line search succeeded
