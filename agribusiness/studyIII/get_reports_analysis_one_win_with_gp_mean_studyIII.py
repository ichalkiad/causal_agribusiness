...
if __name__ == '__main__':
                    ...
                    for mlag in model_lag:
                        ...
                        for report_date in current_dates:                            
                            ...
                            for filein in [filesall[0]]:                                
                                ...
                                # ignore very first window
                                init_window_dates = np.array([pd.Timestamp(np.datetime64(i)).date() for i in np.vstack(dat["rejectnull"][1:,1]).flatten().tolist()])      
                                # get window with left side closest to report publication date
                                closest_win_start_idx = np.argmin(abs(init_window_dates - dateTstamp))
                                # +1 due to ignoring first window
                                jj = closest_win_start_idx + 1
                                ####################################################################################################
                                fileinmedianofmeans = filein.replace(".mat", "_medianofmeans.mat")
                                datmeans = loadmat("{}/{}".format(readdirin, fileinmedianofmeans))
                                ...
                                oneminusp.append(1 - dat["rejectnull"][jj][3][0][0])
                                tstats.append(dat["rejectnull"][jj][4][0][0])
                                ...
                                rdays.append(dat["rejectnull"][jj][0][0])
    ...
    pdout = pd.DataFrame.from_dict({"year" : year,
                                    ...
                                    "gp_mean": gp_mean,
                                    "win_start": win_start})
    pdout.to_csv("{}studyI_data_single_window_gp_mean.csv".format(DIR_out), index=False)
