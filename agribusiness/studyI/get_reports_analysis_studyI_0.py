...
if __name__ == '__main__':
            ...
            for contract in contracts:
                ...
                for report in reports:
                    ...
                    for mlag in model_lag:
                        ...
                        current_files = [i for i in os.listdir(readdirin) if process in i and "forecast_variances" not in i and "medianofmeans" not in i]
                        ...
                        for report_date in current_dates:
                            ...
                            filesall = [j for j in current_files if report_date in j and "M1" not in j and "progress" not in j and "lag{}".format(mlag) in j 
                                           and "_nodata" not in j and "meancov" in j and "forecast_variances" not in j]
                            if len(filesall) == 0:
                            ...
                            for filein in [filesall[0]]:                                
                                ...
                                for jj in range(1,len(dat["rejectnull"]),1):
                                    try:
                                        if dat["rejectnull"][jj][3][0][0] <= 0.05:
                                            reject.append(1)                                            
                                        else:
                                            reject.append(0)
                                    except:                                        
                                        if len(dat["rejectnull"][jj][3]) == 0 or len(dat["rejectnull"][jj][3][0]) == 0:                                            
                                            print(filein)
                                            continue
    ...
    pdout = pd.DataFrame.from_dict({"year" : year,
                                    ...
                                    "reportdate" : rdays})
    pdout.to_csv("{}studyI_data.csv".format(DIR_out), index=False)